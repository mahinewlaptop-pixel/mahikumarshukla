...
export default function Tech({skills, heading}) {
    ...
    for(let skill in skills){
        newSkills.push(
           <div key={skill} className="h-26 w-27 sm:h-56 sm:w-52 lg:h-56 lg:w-52 md:h-56 md:w-52 flex flex-col items-center justify-center m-10 hover:shadow-lg hover:shadow-black rounded-lg">
                <img className=" rounded-3xl h-24 sm:h-24 md:h-24 lg:h-24" src={skills[skill]} alt={skill}/>
                <p className="capitalize my-6 font-serif text-xl md:text-2xl lg:text-2xl">{skill}</p>
           </div>
        );
    };
  return (
